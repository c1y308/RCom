...
void from_json(const json& j, ClassicConfig& classic_conf) {
    if (j.contains("groups")) classic_conf.groups = j.at("groups").get<std::vector<SchedGroup>>();
}

void from_json(const json& j, SchedulerConfig& scheduler_conf) {
    ...
    if (j.contains("classic_conf")) scheduler_conf.classic_conf = j.at("classic_conf").get<ClassicConfig>();
}

void from_json(const json& j, MainConfig& main_config) {
    if (j.contains("scheduler_conf")) j.at("scheduler_conf").get_to(main_config.scheduler_conf);
}

bool get_main_config_from_file(const std::string& file_path, MainConfig* config){
    if (config == nullptr) {
        AERROR << "Error: MainConfig pointer is null.";
        return false;
    }
