...
Time& Time::Time::operator=(const Time& other){
...
uint64_t Time::to_nanoseconds() const{
     return nanoseconds_; 
}

uint64_t Time::to_microseconds() const{
...
//睡眠当前线程到指定的时间点
void Time::sleep_until(const Time& time) {
  auto nano = std::chrono::nanoseconds(time.to_nanoseconds());
  system_clock::time_point tp(nano);
...
Time Time::operator+(const Duration& rhs) const {
  return Time(nanoseconds_ + rhs.to_nanoseconds());
}
